...
不少介绍 Nginx 的书籍都会选择从源码编译的方式，然而这对新手极不友好。简单概括来说：Nginx 是模块化的，从源码编译时可以选配各种特殊的模块、加入社区提供的丰富的插件等，提供了极高程度的自定义。源码编译的 Nginx 可以用于嵌入式开发等各种场景，但**对于初学者来说，按照[官方安装指南](https://nginx.org/en/linux_packages.html)在 Linux 上安装即可**。
...
使用 apt 源安装 Nginx 时可能会自动创建 `nginx` 用户，也会附带 service 文件。让我们先启动 Nginx 服务：
...
可以看到，Nginx 以 `-c /etc/nginx/nginx.conf` 参数启动，这是它的默认配置文件，同时可以看到其进程树，有 `master` 和 `worker` 进程。对于每个网络连接请求，`master` 会新建一个 `worker` 进程进行处理，从而提高并发性能。
...
如果使用系统服务启动 Nginx，则应当使用 `systemctl` 控制 Nginx。
...
- **简单指令**的参数以**空格**分隔，以 `;` 结尾。
- 如果指令包含空格，一定要用单/双引号引起来。
- **块指令**以花括号包裹的一系列指令结尾，如 `location`。
- 如果块指令中可以有其他块指令，则称为**上下文**。
- `#` 注释

> 例：在下面的配置文件中
>
> - 整个配置文件称为 `main` 上下文
> - `events` 和 `http` 在 `main` 上下文中，`server` 在 `http` 中，`location` 在 `server` 中。

```nginx
user nginx; # 执行 Nginx 进程的用户
worker_processes auto; # Worker 并发数量
...
请参考附带的默认配置文件：`/etc/nginx/conf.d/default.conf`
...
从抽象的角度看，配置 Nginx 就是定义它处理哪些 URL、如何处理对这些 URL 上的资源的 HTTP 请求。从实现的角度看，就是定义一系列的虚拟服务器，它们控制特定域名或 IP 地址的处理。

- `location`

`location` 控制一系列 URL 的处理。每个 `location` 可以代理或者返回一个文件。此外，URL 能被更改，重定向到其他位置或虚拟服务器；也可以返回特定错误码或错误页面。
