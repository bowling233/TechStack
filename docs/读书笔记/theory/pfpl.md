...
\[
\frac{}{\mathsf{nil list}} \quad (2.7a)
\]

\[
\frac{a \mathsf{ nat} \quad b \mathsf{ list}}{\mathsf{cons}(a;b) \mathsf{ list}} \quad (2.7b)
\]
...
!!! example
    ...
    \[
    \frac{}{\mathsf{zero even}} \quad (2.8a)
    \]

    \[
    \frac{b \mathsf{ odd}}{\mathsf{succ}(b) \mathsf{ even}} \quad (2.8b)
    \]

    \[
    \frac{a \mathsf{ even}}{\mathsf{succ}(a) \mathsf{ odd}} \quad (2.8c)
    \]

这些规则的规则归纳原理表明，要同时证明 \(P(a)\) 对于 \(a\) 是偶数时成立，且 \(Q(b)\) 对于 \(b\) 是奇数时成立，只需证明以下几点：

1. \(P(\mathsf{zero})\)；
2. 如果 \(Q(b)\) 成立，则 \(P(\mathsf{succ}(b))\) 成立；
3. 如果 \(P(a)\) 成立，则 \(Q(\mathsf{succ}(a))\) 成立。

!!! example "例如，我们可以使用同时规则归纳来证明：（1）如果 \(a\) 是偶数，则 \(a\) 要么是 0，要么是 \( \mathsf{succ}(b) \)，其中 \(b\) 是奇数；（2）如果 \(a\) 是奇数，则 \(a\) 是 \( \mathsf{succ}(b) \)，其中 \(b\) 是偶数。"

我们定义 \(P(a)\) 当且仅当 \(a\) 是 0 或 \(a\) 是 \( \mathsf{succ}(b) \)，其中 \(b\) 是奇数时成立，并定义 \(Q(b)\) 当且仅当 \(b\) 是 \( \mathsf{succ}(a) \)，其中 \(a\) 是偶数时成立。通过规则归纳可以证明：

1. \(P(\mathsf{zero})\) 成立，因为 \( \mathsf{zero} \) 是 0；
2. 如果 \(Q(b)\) 成立，则 \( \mathsf{succ}(b) \) 是 \( \mathsf{succ}(b') \)，其中 \(b' = b\)，并应用归纳假设；
3. 如果 \(P(a)\) 成立，则 \( \mathsf{succ}(a) \) 是 \( \mathsf{succ}(a') \)，其中 \(a' = a\)，并应用归纳假设。

#### 通过规则定义函数

归纳定义的一个常见用途是通过给定输入和输出之间的图的归纳定义来定义函数，然后证明该关系唯一地确定了给定输入的输出。例如，我们可以定义自然数上的加法函数为关系 \( \mathsf{sum}(a; b; c) \)，其意图是表示 \(c\) 是 \(a\) 和 \(b\) 的和，定义如下：

\[
\frac{b \mathsf{ nat}}{\mathsf{sum(zero; b; b)}} \quad (2.9a)
\]

\[
\frac{\mathsf{sum}(a; b; c)}{\mathsf{sum(succ}(a); b; \mathsf{succ}(c))} \quad (2.9b)
\]
...
!!! note "定理 2.4: 和函数的存在性和唯一性"
    ...
    **定理 2.4** 对于每个自然数 \(a\) 和 \(b\)，存在唯一的自然数 \(c\)，使得 \( \mathsf{sum}(a; b; c) \) 成立。
    ...
    **证明** 该证明分为两部分：
    ...
    对于存在性，我们令 \(P(a)\) 表示以下命题：如果 \(b\) 是自然数，则存在自然数 \(c\)，使得 \( \mathsf{sum}(a; b; c) \) 成立。我们通过规则归纳来证明，如果 \(a\) 是自然数，则 \(P(a)\) 成立。我们有两个情况需要考虑：

    - **规则 (2.2a)**：我们要证明 \(P(\mathsf{zero})\)。假设 \(b\) 是自然数，并令 \(c = b\)，根据规则 (2.9a)，我们得出 \( \mathsf{sum(zero; b; c)} \)。
    - **规则 (2.2b)**：假设 \(P(a)\) 成立，我们要证明 \(P(\mathsf{succ}(a))\)。也就是说，我们假设如果 \(b\) 是自然数，则存在自然数 \(c\)，使得 \( \mathsf{sum}(a; b; c) \) 成立，并且要证明如果 \(b' \) 是自然数，则存在 \(c'\)，使得 \( \mathsf{sum(succ}(a); b'; c') \) 成立。为此，假设 \(b' \) 是自然数。根据归纳假设，存在 \(c\) 使得 \( \mathsf{sum}(a; b'; c) \) 成立。令 \(c' = \mathsf{succ}(c)\)，并应用规则 (2.9b)，我们得出 \( \mathsf{sum(succ}(a); b'; c') \)，如所需。

    ---

    对于唯一性，我们证明，如果 \( \mathsf{sum}(a; b; c_1) \) 成立，且 \( \mathsf{sum}(a; b; c_2) \) 成立，则 \(c_1 = c_2\)，通过基于规则 (2.9) 的规则归纳来证明。

    - **规则 (2.9a)**：我们有 \(a = 0\) 且 \(c_1 = b\)。通过相同规则的内归纳，我们可以证明如果 \( \mathsf{sum(zero; b; c_{2})} \) 成立，则 \(c_2 = b\)。根据引理 2.2，我们得出 \(b = b\)。
...
\[
X \vdash_R J \quad \mathsf{当且仅当} \quad Y \vdash_R J
\]
