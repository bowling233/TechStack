...
在进行 RDMA 通信前，通信双方需要交换 R_Key 和 QP 等信息。一般会先通过以太网建立 TCP 连接等方式进行。如果 TCP/IP 协议栈不可用，可以使用 `rdma_cm`，它的接口与 socket 类似：
...
RDMA Verbs 与 Socket 类似，提供的抽象层次比 IB Verbs 更高，代码量更少。在 RDMA Verbs 的背后，执行的也是 QP 和 CQ 等操作。
