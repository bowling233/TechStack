...
- 在 `thread` 被销毁前，必须保证已调用 `join()` 或 `detach()`。否则析构函数会调用 `std::terminate()`，导致程序终止。
    - 为了保证在异常情况下也能正确销毁线程，可以使用 RAII 技术，将线程对象封装在一个类 `thread_guard` 中，析构函数中调用 `join()`。
...
- 向线程传递参数的步骤：先复制到线程内部存储空间，然后当作临时变量，以**右值**形式传给新线程上的函数。
    - 转换在进入新线程的上下文后才发生。注意传递指针，如：
        ...
        在 `buffer` 从 `char*` 转换为 `std::string` 前，`buffer` 可能被销毁。解决方法是提前转换
